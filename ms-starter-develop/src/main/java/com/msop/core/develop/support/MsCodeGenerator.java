package com.msop.core.develop.support;
...
import com.msop.core.develop.constant.DevelopConstant;
...
@Data
@Slf4j
public class MsCodeGenerator {
    /**
     * 代码所在系统
     */
    private String systemName = DevelopConstant.FRONT;
    /**
     * 代码模块名称
     */
     ...
     * 代码所在服务名
     */
    private String serviceName = "msop-service";
    /**
     * 代码生成的包名
     */
    private String packageName = "com.msop.test";
    /**
     * 代码后端生成的地址
     */
     ...
     * 需要去掉的表前缀
     */
    private String[] tablePrefix = {"msop_"};
    /**
     * 需要生成的表名(两者只能取其一)
     */
    private String[] includeTables = {"msop_test"};
    /**
     * 需要排除的表名(两者只能取其一)
     */
    ...
    public void run() {
        ...
        if (Func.isNotBlank(packageWebDir)) {
            customFile.put("api.js", "/templates/front/api.js.vm");
            customFile.put("crud.vue", "/templates/front/crud.vue.vm");
        ...
        FastAutoGenerator.create(url, username, password)
                ...
                .strategyConfig(builder -> builder.addTablePrefix(tablePrefix).addInclude(includeTables).addExclude(excludeTables)
                        .entityBuilder().naming(NamingStrategy.underline_to_camel).columnNaming(NamingStrategy.underline_to_camel).enableLombok().superClass("com.msop.core.mp.base.BaseEntity").addSuperEntityColumns(superEntityColumns).fileOverride()
                        .serviceBuilder().superServiceClass("com.msop.core.mp.base.BaseService").superServiceImplClass("com.msop.core.mp.base.BaseServiceImpl").formatServiceFileName("I%sService").formatServiceImplFileName("%sServiceImpl").fileOverride()
                        ...
                        .controllerBuilder().superClass("com.msop.core.boot.ctrl.MsController").formatFileName("%sController").enableRestStyle().enableHyphenStyle().fileOverride()
                )
    ...
    public String getOutputDir() {
        return (Func.isBlank(packageDir) ? System.getProperty("user.dir") + "/msop-ops/msop-develop" : packageDir) + "/src/main/java";
    }
