...
public class AliossTemplate {
    ...
    @SneakyThrows
    private FileInfo putFile(String fileName, MultipartFile file) {
        return putFile(ossProperties.getBucketName(), fileName, file);
    }

    @SneakyThrows
    private FileInfo putFile(String bucketName, String fileName, MultipartFile file) {
    ...
    public FileInfo putFile(String bucketName, String fileName, InputStream stream) {
        return put(bucketName, fileName, stream, false);
    }

    @SneakyThrows
    public FileInfo put(String bucketName, String fileName, InputStream stream, boolean cover) {
        makeBucket(bucketName);
        String originalName = fileName;
        fileName = getFileName(originalName);
        // 覆盖上传
        if (cover) {
            ossClient.putObject(getBucketName(bucketName), fileName, stream);
        } else {
            PutObjectResult response = ossClient.putObject(getBucketName(bucketName), fileName, stream);
            int retry = 0;
            int retryCount = 5;
        ...
        fileInfo.setLink(fileLink(bucketName, fileName));
        return fileInfo;
    }
