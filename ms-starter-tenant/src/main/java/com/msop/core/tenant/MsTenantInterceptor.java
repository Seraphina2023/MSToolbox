...
public class MsTenantInterceptor extends TenantLineInnerInterceptor {
    /**
     * 租户处理器
     */
     ...
     * 租户配置文件
     */
    private MsTenantProperties tenantProperties;
    /**
     * 超管需要启用租户过滤的表
     */
    private List<String> adminTenantTables = Arrays.asList("msop_top_menu", "msop_dict_biz");
    ...
    protected BinaryExpression andExpression(Table table, Expression where) {
        ...
        // 若是超管则不进行过滤
        if (doTenantFilter(table.getName())) {
            leftExpression = rightExpression = new StringValue(StringConstant.ONE);
        }
