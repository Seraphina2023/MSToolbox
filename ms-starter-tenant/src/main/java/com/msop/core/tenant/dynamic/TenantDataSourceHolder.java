...
import static com.msop.core.tenant.constant.TenantBaseConstant.*;
...
public class TenantDataSourceHolder {
    ...
    public void handleDataSource(String tenantId) {
        ...
        // 为了保证数据完整性，配置后生成数据源缓存，后台便无法修改更换数据源，若一定要修改请迁移数据后重启服务 或自行修改底层逻辑
        if (!keys.contains(tenantId)) {
            TenantDataSource tenantDataSource = getDataSourcce(tenantId);
     ...
     * @return 租户数据源
     */
    private TenantDataSource getDataSourcce(String tenantId) {
        // 不存在租户数据源则返回空，防止缓存击穿
    ...
    private Boolean existDataSource(String tenantId) {
        ...
        // 入欧租户没有配置数据源则会自动使用master数据源，此举是为了避免在没有数据库的时候频繁查询导致缓存击穿
        Boolean exist = CacheUtil.get(TENANT_DATASOURCE_CACHE, TENANT_DATASOURCE_EXIST_KEY, tenantId, Boolean.class, Boolean.FALSE);
        if (exist == null) {
            TenantDataSource tenantDataSource = jdbcTemplate.queryForObject(TENANT_DATASOURCE_SINGLE_STATEMENT, new String[]{tenantId}, new BeanPropertyRowMapper<>(TenantDataSource.class));
