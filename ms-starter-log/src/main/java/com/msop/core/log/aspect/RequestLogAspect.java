...
import com.msop.core.launch.log.MsLogLevel;
...
@ConditionalOnProperty(value = MsLogLevel.LOG_PROPS_PREFIX + ".enabled", havingValue = "true", matchIfMissing = true)
public class RequestLogAspect {
    private final AuditLogProperties properties;

    /**
     * AOP 环切 控制器 Result 返回值
     *
     ...
     */
    @Around(
            "execution(!static com.msop.core.tool.model.Result *(..)) && " +
                    "(@within(org.springframework.stereotype.Controller)) ||" +
                    "(@within(org.springframework.web.bind.annotation.RestController)))"
    )
    public Object aroundApi(ProceedingJoinPoint point) throws Throwable {
        MsLogLevel level = properties.getLevel();
        // 不打印日志，直接返回
        if (MsLogLevel.NONE == level) {
    ...
    public void logIngArgs(ProceedingJoinPoint point, StringBuilder beforeReqLog, List<Object> beforeReqArgs) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            if (value instanceof HttpServletRequest) {
                ...
                paraMap.put(name, StringUtil.removeSuffix(sb.toString(), StringConstant.COMMA));
                continue;
     ...
     * @param beforeReqArgs beforeReqArgs
     */
    public void logIngHeaders(HttpServletRequest request, MsLogLevel level,
                              StringBuilder beforeReqLog, List<Object> beforeReqArgs) {
        // 打印请求头
        if (MsLogLevel.HEADERS.lte(level)) {
