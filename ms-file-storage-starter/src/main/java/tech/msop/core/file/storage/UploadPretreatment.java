...
public class UploadPretreatment {
    ...
    /**
     * MIME 类型，如果不设置则在上传文件根据{@link MultipartFileWrapper#getContentType()}自动识别
     */
     ...
     * 设置文件所属对象ID
     *
     * @param objectId 如果不是String类型会自动调用toString()方法
     ...
     * 可以进行裁剪、旋转、缩放、水印等操作，默认输出格式通过 thumbnailSuffix 获取
     *
     * @param consumer consumer
     * @return UploadPretreatment
     */
    ...
    public UploadPretreatment thumbnail() {
        return thumbnail(th -> th.size(200, 200));
    }
